...
Briefing execution keeps its Plan/Briefing origin above the executor boundary. The executor continues to own only Milestone cycle state, reports, branches, package checks, and recommender behavior. After a terminal result, the TUI planning wrapper clears the origin and best-effort marks only an approved source Briefing completed after reloading and revalidating its Plan. Planning save failures are visible warnings and cannot invalidate the already-recorded Milestone cycle.

Plan orchestration uses the same boundary at queue scale. `plan start` and `plan resume` run in the pre-TUI CLI layer, persist optional coordination checkpoints in the selected typed Plan, and pass immutable queue context through `StartCycleMsg`. The TUI invokes the normal preflight and cycle engine, then calls the planning coordinator with the terminal event's Milestone ID to revalidate identity, reconcile completion, and optionally queue the next ordinary Milestone. Approval and launch callbacks revalidate the live Plan/Briefing/link/index identity, while delayed terminal callbacks retain ownership across runner cancellation navigation. Deterministic generated Milestones can be reclaimed after an interrupted spec/index/link boundary only when their stored identity markers match the retained Plan and Briefing. Uncertain `cycle-running` checkpoints remain durable and non-launchable across repeated resumes. `internal/executor` remains planning-agnostic. Standalone Milestones are never selected by a Plan run merely because they exist in the compact index.
